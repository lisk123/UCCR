...
class ReDialDataset(BaseDataset):
    ...
    def _load_raw_data(self):
        ...
        with open(os.path.join(self.dpath, 'new_new_choiced_last_six_train_data.json'), 'r', encoding='utf-8') as f:
            train_data = json.load(f)
            logger.debug(f"[Load train data from {os.path.join(self.dpath, 'new_new_choiced_last_six_train_data.json')}]")
        with open(os.path.join(self.dpath, 'new_new_choiced_last_six_valid_data.json'), 'r', encoding='utf-8') as f:
            valid_data = json.load(f)
            logger.debug(f"[Load valid data from {os.path.join(self.dpath, 'new_new_choiced_last_six_valid_data.json')}]")
        with open(os.path.join(self.dpath, 'new_new_choiced_last_six_test_data.json'), 'r', encoding='utf-8') as f:
            test_data = json.load(f)
            logger.debug(f"[Load test data from {os.path.join(self.dpath, 'new_new_choiced_last_six_test_data.json')}]")
    ...
    def _data_preprocess(self, train_data, valid_data, test_data):
        ...
        for aaa in all_datas:
            conv_id_list.append(int(aaa['conv_id']))
        import numpy as np
        ...
        augmented_all_convs = [self._merge_conv_data(conversation['dialog']) for conversation in tqdm(sorted_all_data)]
        ...
        for i in tqdm(range(len(sorted_all_data))):
            ...
            current_conv_id = int(sorted_all_data[i]['conv_id'])
