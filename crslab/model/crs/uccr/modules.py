...
class GateLayer_4_eles(nn.Module):
    def __init__(self, input_dim):
        super(GateLayer_4_eles, self).__init__()
        self._norm_layer1 = nn.Linear(input_dim * 4, input_dim)
        self._norm_layer2 = nn.Linear(input_dim, 4)

    def forward(self, input1, input2, input3, input4):
        norm_input = self._norm_layer1(torch.cat([input1, input2, input3, input4], dim=-1))
        gate = F.softmax(self._norm_layer2(norm_input))  # (bs, 1)
        gated_emb = gate[:, 0].view(-1,1) * input1 + gate[:, 1].view(-1,1) * input2 + gate[:, 2].view(-1,1) * input3 + gate[:, 3].view(-1,1) * input4 #(bs, dim)
        return gated_emb
    
class GateLayer_5_eles(nn.Module):
    def __init__(self, input_dim):
        super(GateLayer_5_eles, self).__init__()
        self._norm_layer1 = nn.Linear(input_dim * 5, input_dim)
        self._norm_layer2 = nn.Linear(input_dim, 5)

    def forward(self, input1, input2, input3, input4, input5):
        norm_input = self._norm_layer1(torch.cat([input1, input2, input3, input4, input5], dim=-1))
        gate = F.softmax(self._norm_layer2(norm_input))  # (bs, 1)
        gated_emb = gate[:, 0].view(-1,1) * input1 + gate[:, 1].view(-1,1) * input2 + gate[:, 2].view(-1,1) * input3 + gate[:, 3].view(-1,1) * input4 + gate[:, 4].view(-1,1) * input5 #(bs, dim)
        return gated_emb
