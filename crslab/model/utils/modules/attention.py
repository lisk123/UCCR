...
class SelfAttentionSeq(nn.Module):
    def __init__(self, dim, da, alpha=0.2, dropout=0.5):
        ...
        self.pos_embedding = nn.Embedding(115, 32)
        self.pos_embedding_linear = nn.Linear(32, 32)
        self.pos_a_small = nn.Parameter(torch.zeros(size=(32, 32)))
        self.pos_b_small = nn.Parameter(torch.zeros(size=(32, 1)))
        nn.init.xavier_uniform_(self.pos_a_small.data, gain=1.414)
        nn.init.xavier_uniform_(self.pos_b_small.data, gain=1.414)
        
    def forward(self, h, mask=None, return_logits=False, pos_info=None):
        """
        ...
        if pos_info == None:
            ...
            assert 1==0
            pos_rep = self.pos_embedding(pos_info)
            pos_rep = self.pos_embedding_linear(pos_rep)
            turn_e = torch.matmul(torch.tanh(torch.matmul(pos_rep, self.pos_a_small)), self.pos_b_small).squeeze(dim=1)
            
            pos_mask = pos_info.eq(0)
            pos_full_mask = -1e30 * pos_mask.float()
            pos_batch_mask = torch.sum((pos_mask == False), -1).bool().float().unsqueeze(-1)  # for all padding one, the mask=0
            pos_mask = pos_full_mask * pos_batch_mask
            turn_e += pos_mask.unsqueeze(-1)
            attention = (F.softmax(e, dim=1) + F.softmax(turn_e, dim=1)) / 2
